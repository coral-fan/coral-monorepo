...
const event: EventData = {
  ...
  allowedCollections: ['0xf6a817e0af31d1f8accf9e301c2a9bb08962c160'],
...
const addEvent = async () => {
  ...
  const { artistId, allowedCollections, exclusiveCollections } = event;
  ...
  [...allowedCollections, ...(exclusiveCollections ?? [])].forEach(async (collectionId) => {
    ...
    if (!collectionDocSnapshot.exists) {
      throw Error(`collection with id ${collectionId} doesn't exist.`);
    }
  });
  ...
  if (eventDocSnapshot.exists) {
    throw Error(`event with id ${id} already exists.`);
  }
